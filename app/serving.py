...
class ServingModel:
   ...
   def search(self, query: str, limit: int = 12) -> list[int]:
        ...
        plain substring match fails on the two things users actually type:
        'the godfather' (wrong order) and 'god father' (wrong spacing).
        Matching on stripped tokens handles both, and a squashed no-space
        ...
        if len(idx) == 0:                       # fall back: ignore all spacing
